...
@section('content')
    <div class="max-w-3xl mx-auto mt-8 bg-white p-6 rounded shadow">
        ...
        <form action="{{ route('admin.pegawai.update', $pegawai->id) }}" method="POST">
            ...
            <input name="name" class="w-full border p-2 rounded mb-3" value="{{ old('name', $pegawai->name) }}">

            <label class="block mb-2">Email</label>
            <input name="email" class="w-full border p-2 rounded mb-3" value="{{ old('email', $pegawai->email) }}">

            <label class="block mb-2">Jabatan</label>
            <input name="jabatan" class="w-full border p-2 rounded mb-3" value="{{ old('jabatan', $pegawai->jabatan) }}">

            <label class="block mb-2">No HP</label>
            <input name="no_hp" class="w-full border p-2 rounded mb-3" value="{{ old('no_hp', $pegawai->no_hp) }}">

            <label class="block mb-2">Password (kosongkan jika tidak ingin ganti)</label>
